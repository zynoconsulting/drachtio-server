...
ARG SOURCE_REPO=https://github.com/drachtio/drachtio-server.git
...
RUN apt-get update \
  ...
  && echo "cloning ${SOURCE_REPO} at ${DETECTED_TAG}" \
  && git clone --depth=50 --branch "${DETECTED_TAG}" "${SOURCE_REPO}" /usr/local/src/drachtio-server \
